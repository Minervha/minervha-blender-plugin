...
| `export_limits.py` | **Export guardrails** (net-new). Pure decision logic (`evaluate` soft/hard breaches, `clamp_max_res` = the fixed 8192 px texture rule) + cheap bpy counters (`gather_scene_budget`: faces/unique-geometry via `len(mesh.polygons)`, deduped by `mesh_key`; `gather_texture_inventory`: source images by dimension desc; `dim_histogram`: pure N×-per-dimension breakdown). `DEFAULT_THRESHOLDS` mirrors the prefs. bpy-optional (pure fns tested without Blender) | `bpy` (counters only) | [export-guardrails](../docs/plans/features/export-guardrails/plan.md) |
| `ui.py` | "Minervha" N-panel: **Materials/Scene mode toggle** + scope dropdown + **Scene-only Target dropdown** (Collection / Showroom / NewWildLifeMap / OldWildLifeMap → save `level`) + **Textures box** (Prefer JPG / JPG Quality / **Keep normal maps as PNG** / Max resolution → `tex_opts`; the baker also forces PNG for a baked `normal` channel when set) + Export .wlsave operator + report (sanitized/renamed + scene counters: objects/meshes/no-UV/procedural + collection-vs-map line). **Export guardrails:** `MINERVHA_AddonPreferences` (face/object soft+hard thresholds + `experimental_mode`); pre-flight `evaluate` in `invoke` (hard → ERROR + block before the file dialog) & `_prepare_scene` (defensive hard + soft WARNING toast); `_tex_opts` clamps `max_res` to 8192; **Export budget box** with `UILayout.progress` usage bars (scene faces / unique geometries + per-mesh offender bars) + a **texture resolution histogram** (N× dimension, ⤓-marks >8k) + experimental banner. **Bake default now ON.** **`.wlsave` Name pre-filled from the .blend filename** (`_on_load_prefill_name` `load_post` handler + a one-shot `bpy.app.timers` pass for the already-open file — `bpy.data` is restricted during `register()`; always tracks the .blend name — overwrites any prior value, skips the write when already correct). **Defaults: Scene / Selected Objects / Showroom** | `bpy`, `introspect`, `wlsave_export`, `scene_introspect`, `obj_export`, `export_limits` | chunk-06 (validated live); scene-export chunk-04; filename-sanitization; map-export-target; [texture-options](../docs/plans/features/texture-options/plan.md); [export-guardrails](../docs/plans/features/export-guardrails/plan.md) |
...
**Export guardrails — pre-flight caps + usage bars (validated live Blender 5.1.2).** Scene exports had no
volume guardrail: nothing stopped a multi-million-face / huge-object-count scene from launching a multi-minute
run or shipping a `.wlsave` too heavy for the game, and `Max Resolution` defaulted to no cap. New
`export_limits.py` adds **two-tier** thresholds (soft = warn, hard = block) on **scene faces**, **per-mesh
faces** and **unique geometries** (deduped mesh datablocks), plus a fixed **8192 px texture rule**
(`clamp_max_res`, applied in `_tex_opts` → covers source AND baked textures). Enforcement is **pre-flight**:
`MINERVHA_OT_export_wlsave.invoke` blocks a hard-breach scene with an ERROR **before** the file dialog opens;
`_prepare_scene` re-checks defensively (covers headless `EXEC_DEFAULT`) and emits a soft WARNING toast that
still lets the export proceed. Thresholds live in **`MINERVHA_AddonPreferences`** (global, configurable); an
**`experimental_mode`** toggle disables *everything* (no block, no clamp). The panel gains an **Export budget**
box rendering `UILayout.progress` usage bars (faces / geometries against the hard limit, status-icon coded) +
per-mesh offender bars, a **texture resolution histogram** (`N× dimension`, ⤓-marking those the 8k rule will
downscale), and an experimental banner. The old static "Scene export limits" hint
box was removed (superseded) and **Bake defaults to ON**. Pure logic (`evaluate`/`clamp_max_res`) is unit-tested
without Blender; live-verified on `Cinema_Theatron` (187 k faces / 1382 geometries under defaults → clean; a
...
**Collection & parenting hierarchy (opt-in, default ON — validated live Blender 5.1.2).** The scene export
emitted a flat prop list (hierarchy = object parenting only; Collections drove
...
master group. Out of scope: collection-instance empties. **Validated live (Blender 5.1.2)** —
`tests/verify_collection_hierarchy_live.py` builds nested collections + a view-layer-excluded collection + an
object-parented empty and asserts the bpy readers' tree (nesting, pruning, exclusion drop, object-parent kept)
across SCENE and COLLECTION scope. Plan:
...
max-res paths;
`verify_collection_hierarchy_live.py` is a headless Blender probe building nested collections + a view-layer-
excluded collection + an object-parented empty, asserting the collection-hierarchy bpy readers' prop tree
(nesting / pruning / exclusion drop / object-parent kept) across SCENE and COLLECTION scope.
